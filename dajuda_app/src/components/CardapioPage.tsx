...
const categoriasPermitidas = [
  "Marmita do dia",
  "Marmita clássica",
  "Mix de salada", // Adicionado
  "Bebida",
  "Adicional",
  "Unidade",
];

const categoriasOrdemExibicao = [
  "Marmita do dia",
  "Marmita clássica",
  "Mix de salada",
  "Bebida",
...
const CardapioPage: React.FC = () => {
    ...
    try {
      ...
      if (errors.length > 0) {
        ...
        setItensCardapio(prev => prev.map(i => {
            const savedItem = itemsComAlteracoes.find(altered => altered.id === i.id);
            if (savedItem) {
            ...
            return {...i, isEditing: false }; // Garante que todos os isEditing sejam false
        }));
      }
    } catch (e) {
  ...
  return (
    <div className="container mx-auto p-4 pb-20">
      ...
      {categoriasOrdemExibicao.map(categoriaNome => {
        const itensDaCategoria = groupedItens[categoriaNome];
        if (!itensDaCategoria || itensDaCategoria.length === 0) {
          return null;
        }
        return (
          <div key={categoriaNome} className="mb-8">
            <h3 className="text-2xl font-semibold text-pink-600 mb-4 border-b-2 border-pink-200 pb-2 capitalize">{categoriaNome.toLowerCase().replace("_", " ")}</h3>
                  ...
                  {item.isEditing ? (
                    <div className="space-y-3">
                      <div>
                        ...
                        <input type="text" value={item.nome_produto} onChange={(e) => handleEditInputChange(item.id, 'nome_produto', e.target.value)} className="block w-full p-2 border-2 border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500 text-sm bg-white" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-0.5">Categoria</label>
                        <select value={item.categoria} onChange={(e) => handleEditInputChange(item.id, 'categoria', e.target.value)} className="block w-full p-2 border-2 border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500 text-sm bg-white">
                          {categoriasPermitidas.map(cat => (<option key={cat} value={cat}>{cat}</option>))}
                        </select>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-0.5">Disponível</label>
                        <select value={item.disponivel ? 'true' : 'false'} onChange={(e) => handleEditInputChange(item.id, 'disponivel', e.target.value === 'true')} className={`block w-full p-2 border-2 border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500 text-sm ${item.disponivel ? 'bg-green-50' : 'bg-red-50'}`}>
                          <option value="true">Sim</option>
                          <option value="false">Não</option>
                        </select>
                      </div>
                      <div>
                        ...
                        <textarea value={item.descricao_produto || ''} onChange={(e) => handleEditInputChange(item.id, 'descricao_produto', e.target.value)} rows={2} className="block w-full p-2 border-2 border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500 text-sm bg-white" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-0.5">Observação</label>
                        <textarea value={item.observacao || ''} onChange={(e) => handleEditInputChange(item.id, 'observacao', e.target.value)} rows={1} className="block w-full p-2 border-2 border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500 text-sm bg-white" />
                      </div>
                    ...
                    <div className="flex flex-col justify-between h-full">
                        <div>
                            <h4 className="text-xl font-semibold text-gray-800 mb-1">{item.nome_produto}</h4>
...
// .input-field-edit foi substituído por classes diretas nos elementos para maior controle e evitar dependência de CSS externo não aplicado.
// Se preferir usar @apply, a definição abaixo pode ser usada e as classes diretas removidas dos inputs/selects/textareas no modo de edição.
// .input-field-edit {
//   @apply block w-full p-2 border-2 border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500 text-sm bg-white;
// }
