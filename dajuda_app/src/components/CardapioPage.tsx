...
export interface CardapioItem {
  id: number;
  categoria: string;
  disponivel: "Sim" | "Não"; // Alterado para string enum
  nome_produto: string;
  ...
  originalDisponivel?: "Sim" | "Não"; // Alterado para string enum
  originalDescricao?: string | null;
  originalObservacao?: string | null;
}

// Nomes exatos das categorias conforme solicitado e como devem estar no Supabase
...
const CardapioPage: React.FC = () => {
  ...
  const [novoItem, setNovoItem] = useState<Partial<Omit<CardapioItem, 'id' | 'disponivel'>> & { disponivel: "Sim" | "Não" }>({
    nome_produto: '',
    categoria: categoriasDefinidas[0],
    disponivel: "Sim", // Default para "Sim"
    descricao_produto: '',
    observacao: '',
  });
  ...
  const fetchItensCardapio = useCallback(async (source?: string) => {
    ...
    const { data, error: fetchError } = await supabase
      .from('Cárdapio')
      .select('*')
      // A ordenação no Supabase ajuda, mas a ordem final das categorias é controlada por categoriasDefinidas
    ...
    if (fetchError) {
      ...
      // Garantir que 'disponivel' seja "Sim" ou "Não"
      const mappedData = data.map(item => ({
        ...item,
        disponivel: item.disponivel === 'Sim' || item.disponivel === true ? 'Sim' : 'Não', // Garante consistência
      })) as CardapioItem[];
  ...
  const handleAddNewItem = async (e: React.FormEvent) => {
    ...
    const itemParaSalvar = {
        ...
        disponivel: novoItem.disponivel, // Já é "Sim" ou "Não"
      ...
      prevItens.map(item => {
        if (item.id === itemId) {
          if (!item.isEditing) {
            ...
            // Reverter para original ao cancelar
            return {
              ...item,
              isEditing: false,
  ...
  const handleSaveAllChanges = async () => {
    ...
    const updates = itemsComAlteracoes.map(item => {
      ...
      // 'disponivel' aqui já é "Sim" ou "Não"
      return supabase
        .from('Cárdapio')
    ...
    try {
      ...
      if (errors.length > 0) {
        ...
        setItensCardapio(prev => prev.map(i => {
            ...
            if (savedItem) {
                // Limpar campos originais após salvar
...
// CSS classes (btn-primary, btn-secondary, btn-danger, btn-icon) são usadas via Tailwind @apply ou devem ser definidas globalmente.
// A classe .input-field foi substituída por classes Tailwind diretas (formFieldClasses) para consistência.
